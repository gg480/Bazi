...
class YijingAnalyzer {
   ...
   * @param {string} inputData.direction - 起卦方位（可选）
   * @param {number} inputData.user_id - 用户ID
   * @param {Object} inputData.birth_data - 出生数据（可选）
   * @param {Object} inputData.bazi_data - 八字数据（可选）
   * @param {Object} inputData.ziwei_data - 紫微数据（可选）
   * @param {Object} inputData.personal_info - 个人信息（可选）
  ...
  performYijingAnalysis(inputData) {
    try {
      ...
      const { question, direction, user_id, birth_data, bazi_data, ziwei_data, personal_info, divination_method = 'plum_blossom' } = inputData;
      ...
      // 验证起卦方法
      const validMethods = ['time', 'plum_blossom', 'coin', 'number', 'six_lines'];
      if (!validMethods.includes(divination_method)) {
        throw new Error(`输入数据无效：不支持的起卦方法 ${divination_method}`);
      }
      
      // 检查缓存（易经分析时效性较短，缓存时间较短）
      const cacheKey = `yijing:${question}:${divination_method}:${direction || ''}`;
      const cachedResult = this.cache.get(cacheKey, inputData);
      if (cachedResult) {
        return cachedResult;
      }
      ...
      // 根据不同方法起卦
      const hexagramData = this.generateHexagramByMethod(divination_method, currentTime, user_id, question, direction);
      ...
      // 动态分析：基于问题类型和时间因素，结合八字和紫微数据
      const dynamicAnalysis = this.generateDynamicAnalysis(mainHexagramInfo, changingHexagramInfo, question, currentTime, bazi_data, ziwei_data, personal_info);
      
      // 象数分析（结合方位信息）
      const numerologyAnalysis = this.performNumerologyAnalysis(hexagramData, currentTime, question, direction);
      
      // 综合指引（结合八字、紫微数据）
      const overallGuidance = this.generateOverallGuidance(mainHexagramInfo, changingHexagramInfo, question, bazi_data, ziwei_data, personal_info);
      ...
      const result = {
        ...
        detailed_analysis: {
          ...
          practical_application: this.generatePracticalApplication(mainHexagramInfo, changingHexagramInfo, bazi_data, ziwei_data)
        },
        dynamic_guidance: dynamicAnalysis,
        divination_wisdom: {
          key_message: this.generateKeyMessage(mainHexagramInfo, changingHexagramInfo, question, bazi_data, ziwei_data),
          action_advice: this.generateActionAdvice(mainHexagramInfo, changingHexagramInfo, currentTime, bazi_data, ziwei_data),
          timing_guidance: this.generateTimingGuidance(mainHexagramInfo, currentTime, bazi_data),
          philosophical_insight: this.generatePhilosophicalInsight(mainHexagramInfo, changingHexagramInfo)
        }
      };
      
      // 存储到缓存
      this.cache.set(cacheKey, inputData, result);
      return result;
  ...
  // 根据不同方法生成卦象
  generateHexagramByMethod(method, currentTime, userId, question, direction) {
    switch (method) {
      case 'time':
        return this.generateHexagramByTime(currentTime, userId, direction);
      case 'plum_blossom':
        return this.generateHexagramByPlumBlossom(currentTime, question, direction);
      case 'coin':
        return this.generateHexagramByCoin(direction);
      case 'six_lines':
        return this.generateHexagramBySixLines(currentTime, question, direction);
      case 'number':
        return this.generateHexagramByNumber(currentTime, userId, direction);
      default:
        return this.generateHexagramByPlumBlossom(currentTime, question, direction);
    }
  }
  ...
  // 梅花易数时间起卦法（结合方位）
  generateHexagramByTime(currentTime, userId, direction) {
    ...
    // 方位因子（根据方位调整起卦）
    const directionFactor = this.getDirectionFactor(direction);
    
    const upperTrigramNum = (year + month + day + userFactor + directionFactor) % 8 || 8;
    const lowerTrigramNum = (year + month + day + hour + minute + userFactor + directionFactor) % 8 || 8;
    const changingLinePos = (year + month + day + hour + minute + userFactor + directionFactor) % 6 + 1;
    ...
    return {
      ...
      method: '梅花易数时间起卦法' + (direction ? `（方位：${direction}）` : ''),
  ...
  // 梅花易数外应起卦法（结合方位）
  generateHexagramByPlumBlossom(currentTime, question, direction) {
    ...
    const upperTrigramNum = (questionLength + timeSum + directionFactor) % 8 || 8;
    const lowerTrigramNum = (questionLength * 2 + timeSum + directionFactor) % 8 || 8;
    const changingLinePos = (questionLength + timeSum + directionFactor) % 6 + 1;
    ...
    return {
      mainHex: mainHexNumber,
      changingLines: [changingLinePos],
      method: '梅花易数外应起卦法' + (direction ? `（方位：${direction}）` : ''),
      upperTrigram: upperTrigramNum,
      lowerTrigram: lowerTrigramNum,
      direction: direction || '未指定'
    };
  ...
  // 增强金钱卦起卦法（结合方位）
  generateHexagramByCoin(direction) {
    ...
    return {
      ...
      method: hexagramData.method + (direction ? `（方位：${direction}）` : ''),
  ...
  // 六爻起卦法（新增）
  generateHexagramBySixLines(currentTime, question, direction) {
    const timeSum = currentTime.getHours() + currentTime.getMinutes() + currentTime.getSeconds();
    const questionLength = question ? question.length : 8;
    const directionFactor = this.getDirectionFactor(direction);
    
    // 使用时间、问题和方位生成六个爻
    const upperTrigramNum = (timeSum + directionFactor) % 8 || 8;
    const lowerTrigramNum = (questionLength + directionFactor) % 8 || 8;
    
    // 生成变爻
    const changingLinePos = (timeSum + questionLength + directionFactor) % 6 + 1;
    // 50%概率产生第二个变爻
    const secondChangingLine = Math.random() > 0.5 ? ((timeSum + questionLength + directionFactor + 3) % 6 + 1) : null;
    
    const mainHexNumber = this.getHexagramNumber(upperTrigramNum, lowerTrigramNum);
    const changingLines = secondChangingLine && secondChangingLine !== changingLinePos 
      ? [changingLinePos, secondChangingLine].sort((a, b) => a - b)
      : [changingLinePos];
    
    return {
      mainHex: mainHexNumber,
      changingLines: changingLines,
      method: '六爻起卦法' + (direction ? `（方位：${direction}）` : ''),
      upperTrigram: upperTrigramNum,
      lowerTrigram: lowerTrigramNum,
      direction: direction || '未指定'
    };
  ...
  // 数字起卦法
  // 增强数字起卦法（结合高质量随机数）
  generateHexagramByNumber(currentTime, userId, direction) {
    ...
    const upperTrigramNum = (Math.floor(timeNum / 1000) + userNum + randomFactor + directionFactor) % 8 || 8;
    const lowerTrigramNum = (Math.floor(timeNum / 100) + userNum * 2 + randomFactor * 2 + directionFactor) % 8 || 8;
    const changingLinePos = (timeNum + userNum + randomFactor + directionFactor) % 6 + 1;
    ...
    return {
      mainHex: mainHexNumber,
      changingLines: [changingLinePos],
      method: '增强数字起卦法' + (direction ? `（方位：${direction}）` : ''),
      upperTrigram: upperTrigramNum,
      lowerTrigram: lowerTrigramNum,
  ...
  // 动态分析：基于问题类型和时间因素
  generateDynamicAnalysis(mainHex, changeHex, question, currentTime, baziData, ziweiData, personalInfo) {
    ...
    return {
      ...
      targeted_guidance: this.generateTargetedGuidance(mainHex, changeHex, questionType, timeFactors, baziData, ziweiData),
      practical_advice: this.generatePracticalAdvice(mainHex, changeHex, questionType),
      personal_insight: baziData || ziweiData ? this.generatePersonalInsight(baziData, ziweiData, personalInfo) : null
    };
  }
  ...
  // 分析动爻数理
   ...
   generatePracticalNumerologyApplication(totalNum, question) {
     return {
       ...
       success_factors: this.getSuccessFactorsSimple(totalNum)
     };
   }
  ...
  // 生成关键信息
  generateKeyMessage(mainHex, changeHex, question, baziData, ziweiData) {
    let baseMessage = mainHex.keyMessage || '顺应自然，把握时机';
    
    // 结合八字信息
    if (baziData?.day_master) {
      baseMessage = `作为${baziData.day_master}日主，${baseMessage}`;
    }
  ...
  // 生成行动建议（结合八字、紫微）
  generateActionAdvice(mainHex, changeHex, currentTime, baziData, ziweiData) {
    ...
    // 结合八字
    if (baziData?.day_master) {
      advice += ` 结合您${baziData.day_master}日主的特质，`;
    }
    
    // 结合紫微
    if (ziweiData?.ming_gong_stars?.length > 0) {
      advice += ` 命宫主星${ziweiData.ming_gong_stars.join('、')}提示您，`;
    }
  ...
  // 生成个性化洞察（结合八字、紫微）
  generatePersonalInsight(baziData, ziweiData, personalInfo) {
    const insights = [];
    
    // 八字洞察
    if (baziData) {
      if (baziData.day_master) {
        insights.push(`作为${baziData.day_master}日主，您具有独特的性格特质和行事风格`);
      }
      
      if (baziData.wuxing_analysis?.element_distribution) {
        const wuxing = baziData.wuxing_analysis.element_distribution;
        const strongElements = Object.entries(wuxing)
          .filter(([_, count]) => Number(count) >= 2)
          .map(([elem]) => elem);
        
        if (strongElements.length > 0) {
          insights.push(`五行${strongElements.join('、')}较旺，这会影响您对卦象的感应和解读`);
        }
      }
    }
    ...
    // 紫微洞察
    if (ziweiData) {
      const stars = ziweiData.ming_gong_stars || [];
      if (stars.length > 0) {
        insights.push(`命宫主星${stars.join('、')}构成了您的基本人格，这决定了您面对挑战时的方式`);
      }
      
      if (ziweiData.wuxing_ju) {
        insights.push(`您的五行局为${ziweiData.wuxing_ju.type}，大限每${ziweiData.wuxing_ju.number * 10}年一步，这会影响运势周期`);
      }
    }
    ...
    if (personalInfo) {
      if (personalInfo.name) {
        insights.push(`${personalInfo.name}，这是您的个人卦象解读，请结合自身实际情况理解`);
      }
    }
    
    return insights.join('；') || '结合您的个人特征，卦象提供了专属的指引';
  }

  // 生成时机指导（结合八字）
  generateTimingGuidance(mainHex, currentTime, baziData) {
    let timing = mainHex.guance || '把握时机，顺势而为';
    const hour = currentTime.getHours();
    const month = currentTime.getMonth() + 1;
    ...
    // 结合八字日主
    if (baziData?.day_master) {
      const dayMasterTiming = this.getDayMasterTiming(baziData.day_master);
      guidance += ` ${dayMasterTiming}`;
    }
  ...
  // 生成针对性指导
  generateTargetedGuidance(mainHex, changeHex, questionType, timeFactors, baziData, ziweiData) {
    let guidance = `针对您关于${questionType.type}的问题，`;
    guidance += `本卦【${mainHex.name}】在${questionType.focus}方面的指示是：${mainHex.guidance}。`;
    
    // 结合八字信息
    if (baziData) {
      guidance += ` 结合您的八字特征（日主${baziData.day_master}），`;
    }
    
    // 结合紫微信息
    if (ziweiData?.ming_gong_stars?.length > 0) {
      guidance += ` 考虑到命宫主星${ziweiData.ming_gong_stars.join('、')}的影响，`;
    }
  ...
  // 生成综合指引（结合八字、紫微数据）
  generateOverallGuidance(mainHex, changingHex, question, baziData, ziweiData, personalInfo) {
    let guidance = '';
    
    // 基础卦象指引
    guidance += `根据【${mainHex.name}卦】的启示，`;
    
    // 结合八字数据
    if (baziData) {
      const dayMaster = baziData.day_master;
      const wuxing = baziData.wuxing_analysis?.element_distribution;
      if (dayMaster) {
        guidance += `您是${dayMaster}日主，`;
      }
      if (wuxing) {
        const strongElements = Object.entries(wuxing)
          .filter(([_, count]) => Number(count) >= 2)
          ...
          .join('、');
        if (strongElements) {
          guidance += `五行中${strongElements}较旺，`;
        }
      }
    }
    ...
    // 结合紫微数据
    if (ziweiData) {
      const mingGongStars = ziweiData.ming_gong_stars || [];
      if (mingGongStars.length > 0) {
        guidance += `命宫主星${mingGongStars.join('、')}，`;
      }
    }
    
    // 综合建议
    guidance += `关于"${question}"的问题，`;
    
    if (changingHex) {
      guidance += `从${mainHex.name}到${changingHex.name}的变化显示，`;
    }
    
    guidance += `${mainHex.guidance} `;
    
    // 根据卦象性质给出个性化建议
    if (baziData || ziweiData) {
      guidance += `结合您的命理特征，建议您${this.getPersonalizedAdvice(mainHex, baziData, ziweiData)}。`;
    } else {
      guidance += `请根据卦象指引行动。`;
    }
  ...
  // 生成个性化建议
  getPersonalizedAdvice(hex, baziData, ziweiData) {
    const adviceParts = [];
    ...
    if (baziData?.wuxing_analysis?.element_distribution) {
      ...
      // 木旺的人建议
      if ((Number(wuxing['木'])) >= 2) {
        adviceParts.push('发挥您的创造力和灵活性');
      }
      
      // 火旺的人建议
      if ((Number(wuxing['火'])) >= 2) {
        adviceParts.push('保持热情但要控制冲动');
      }
      
      // 土旺的人建议
      if ((Number(wuxing['土'])) >= 2) {
        adviceParts.push('发挥您的稳重和坚持');
      }
      
      // 金旺的人建议
      if ((Number(wuxing['金'])) >= 2) {
        adviceParts.push('发挥您的决断力和执行力');
      }
      
      // 水旺的人建议
      if ((Number(wuxing['水'])) >= 2) {
        adviceParts.push('发挥您的智慧和适应力');
      }
    }
    
    // 根据紫微主星给建议
    if (ziweiData?.ming_gong_stars) {
      const stars = ziweiData.ming_gong_stars;
      
      if (stars.includes('紫微')) {
        adviceParts.push('发挥领导才能，但要注意听取他人意见');
      }
      if (stars.includes('天机')) {
        adviceParts.push('发挥策划能力，但不要过度分析');
      }
      if (stars.includes('太阳')) {
        adviceParts.push('发挥光明磊落的特质，但要注意保护他人');
      }
      if (stars.includes('武曲')) {
        adviceParts.push('发挥执行力，但不要过于强硬');
      }
      if (stars.includes('天同')) {
        adviceParts.push('发挥协调能力，但要有原则');
      }
    }
    ...
    if (adviceParts.length > 0) {
      ...
      return hex.keyMessage;
    }
  }

  // 生成实用建议（结合八字、紫微）
  generatePracticalApplication(mainHex, changingHex, baziData, ziweiData) {
    const application = {
      decision_making: mainHex.guidance || changingHex?.guidance || ''
    };
    
    // 时机策略（结合八字）
    if (baziData) {
      const dayMaster = baziData.day_master;
      application.timing_strategy = this.getTimingAdvice(dayMaster, mainHex);
    } else {
      application.timing_strategy = mainHex.actionAdvice || '时机把握需要根据具体情况判断';
    }
    
    // 成功要素（结合紫微）
    if (ziweiData) {
      application.success_factors = this.getSuccessFactors(ziweiData, mainHex);
    } else {
      application.success_factors = mainHex.keyMessage || '成功需要天时地利人和的配合';
    }
  ...
  // 获取时机建议
  getTimingAdvice(dayMaster, hex) {
    const timingMap = {
      '甲': '建议在春季或甲日行动，适合开创性决策',
      '乙': '建议在春季或乙日行动，适合柔性推进',
      '丙': '建议在夏季或丙日行动，适合展示才华',
      '丁': '建议在夏季或丁日行动，适合精细规划',
      '戊': '建议在四季月或戊日行动，适合稳健发展',
      '己': '建议在四季月或己日行动，适合积累资源',
      '庚': '建议在秋季或庚日行动，适合果断决策',
      '辛': '建议在秋季或辛日行动，适合精细操作',
      '壬': '建议在冬季或壬日行动，适合灵活应变',
      '癸': '建议在冬季或癸日行动，适合深度思考'
    ...
    return timingMap[dayMaster] || hex.actionAdvice;
  }

  // 获取成功要素
  getSuccessFactors(ziweiData, hex) {
    const factors = [];
    const stars = ziweiData.ming_gong_stars || [];
    ...
    if (stars.includes('天机') || stars.includes('太阴')) {
      factors.push('运用智慧');
    }
    if (stars.includes('武曲') || stars.includes('七杀')) {
      factors.push('采取果断行动');
    }
    if (stars.includes('天府')) {
      factors.push('做好充分准备');
    }
    if (stars.includes('天同')) {
      factors.push('协调各方关系');
  ...
  // 简化的成功要素（用于数理分析）
  getSuccessFactorsSimple(totalNum) {
    const successMap = {
      2: '保持平衡，稳步推进',
      3: '积极创新，抓住机遇',
      4: '稳健务实，循序渐进',
    ...
    return successMap[totalNum] || '综合分析，灵活应对';
  }
}
